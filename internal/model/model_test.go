...
func TestUpdate_BroadcastCompletedMsg_WithErrors(t *testing.T) {
	m := New()
	m.mode = ModeBroadcastPrompt
	errs := []string{"/repo/a: directory does not exist"}
	result, cmd := m.Update(broadcastCompletedMsg{errors: errs})
	rm := result.(Model)
	if rm.mode != ModeList {
		t.Errorf("expected ModeList, got %v", rm.mode)
	}
...
func TestDashboard_CtrlD_ScrollsPreviewDown(t *testing.T) {
	sessions := []session.Session{
		{Name: "s0", Status: session.StatusIdle, WindowIndex: "0"},
		{Name: "s1", Status: session.StatusIdle, WindowIndex: "1"},
	}
...
func TestDashboard_CtrlD_ClampsToZero(t *testing.T) {
	sessions := []session.Session{
		{Name: "s0", Status: session.StatusIdle, WindowIndex: "0"},
	}
	m := testModel(sessions)
...
func TestDashboard_CursorMove_ResetsScrollOffset(t *testing.T) {
	sessions := []session.Session{
		{Name: "s0", Status: session.StatusIdle, WindowIndex: "0"},
		{Name: "s1", Status: session.StatusIdle, WindowIndex: "1"},
	}
